            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse ($ventas as $v)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                        ...
                        <td class="px-4 py-2.5 text-xs text-amber-600">S/ {{ number_format(($v->clientDebt?->saldo_pendiente ?? (($v->total ?? 0) - ($v->monto_inicial ?? 0))), 2) }}</td>
                        ...
                        <td class="px-4 py-2.5 text-right">
                            <div class="inline-flex gap-1">
                                @if ($v->cliente)
                                    ...
                                    <flux:button size="xs" variant="ghost" href="{{ route('pos.index', ['cobrar_cliente' => $v->cliente_id]) }}" wire:navigate>Cobrar</flux:button>
                                @endif
                                @if ($v->clientDebt)
                                    <flux:button size="xs" variant="ghost" href="{{ route('pos.cuentas-por-cobrar') }}?search={{ urlencode($v->cliente?->numero_documento ?? $v->numero_venta) }}" wire:navigate>CxC</flux:button>
                                @endif
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-8 text-center text-xs text-zinc-500 dark:text-zinc-400">No hay ventas a crédito</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
