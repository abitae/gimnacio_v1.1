...
class VentaService
{
    ...
    public function procesarVenta(array $data): Venta
    {
        ...
        return DB::transaction(function () use ($data, $caja, $items) {
            ...
            $baseConIgv = $subtotal - $descuento;
            // IGV incluido: calcular el IGV del monto que ya lo incluye
            // IGV = base * 18/118
            $igv = round($baseConIgv * 18 / 118, 2);
            // El total es el subtotal menos el descuento (ya incluye IGV)
            $total = $baseConIgv;
            ...
            $this->registrarPagoEnCaja($venta, $caja);
     ...
     */
    protected function registrarPagoEnCaja(Venta $venta, Caja $caja): void
    {
