...
/**
 * Alta, edición, eliminación y foto de cliente (ClienteService).
 *
...
trait ManagesClienteCrudAndPhoto
{
    ...
    public array $clienteModalState = [
        'create' => false,
        'delete' => false,
        'photo' => false,
        'salud' => false,
    ...
    public ?int $photoClienteId = null;
    ...
    public function openClientePhotoModal(?int $id = null): void
    {
        ...
        $this->clienteModalState['photo'] = true;
    }

    public function clearCapturedPhoto(): void
    {
    ...
    {
        $this->clienteModalState['salud'] = false;
        $this->saludClienteId = null;
    }

    public function closeClienteCreateModal(): void
    {
        $this->cleanupTemporaryCapturedPhoto();
        $this->clienteModalState['create'] = false;
        $this->resetClienteForm();
    }

    public function closeClienteDeleteModal(): void
    {
        $this->clienteModalState['delete'] = false;
        $this->clienteId = null;
    }

    public function closeClientePhotoModal(): void
    {
        $this->cleanupTemporaryCapturedPhoto();
        $this->clienteModalState['photo'] = false;
    ...
    public function closeClienteModals(): void
    {
        if (isset($this->formData['foto_captured'])) {
            $tempPath = $this->formData['foto_captured'];
            if (Storage::disk('public')->exists($tempPath)) {
    ...
    public function capturePhoto($imageData): void
    {
        try {
            ...
            $path = $this->processAndStoreClienteImageFromBinary($decoded, 'temp');
            if ($path === null) {
    ...
    public function uploadClientePhoto(): void
    {
        ...
        try {
            ...
            if (isset($this->formData['foto_captured'])) {
                ...
                $path = $this->processAndStoreClienteImageFromBinary($binary, 'clientes');
                if ($path === null) {
                ...
                $path = $this->processAndStoreClienteUploadedFile($this->foto);
                if ($path === null) {
            ...
            if (! $this->photoClienteId) {
                ...
                $this->closeClientePhotoModal();

                return;
            }

            $cliente = $this->clienteService->find($this->photoClienteId);
            if (! $cliente) {
                $this->flashToast('error', 'Cliente no encontrado');
            ...
            $clienteActualizado = $this->clienteService->update($this->photoClienteId, ['foto' => $path]);
            ...
            if ($this->selectedClienteId === $this->photoClienteId) {
                ...
                if (method_exists($this, 'refreshSelectedClienteContext')) {
                    $this->refreshSelectedClienteContext($this->photoClienteId);
                }
            }
            ...
            $this->handleClienteValidationErrors($e);
        } catch (\Exception $e) {
    ...
    public function saveCliente(): void
    {
        ...
        try {
            ...
            if ($this->clienteId) {
                ...
                if ($this->selectedClienteId === $this->clienteId && method_exists($this, 'refreshSelectedClienteContext')) {
                    $this->refreshSelectedClienteContext($this->clienteId);
                }
            } else {
                ...
                $this->closeClienteCreateModal();
                if (method_exists($this, 'selectCliente')) {
    ...
    public function deleteCliente(): void
    {
        ...
        try {
            ...
            $this->closeClienteDeleteModal();
            if ($this->selectedClienteId === $deletedId && method_exists($this, 'clearClienteSelection')) {
    ...
    protected function processAndStoreClienteImageFromBinary(string $binary, string $subfolder): ?string
    {
    ...
    protected function processAndStoreClienteUploadedFile($uploadedFile): ?string
    {
        ...
        return $this->processAndStoreClienteImageFromBinary($binary, 'clientes');
    }

    protected function handleClienteValidationErrors(\Illuminate\Validation\ValidationException $e): void
    {
